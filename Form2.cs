...
namespace WinFormsApp1
{
    public partial class Form2 : Form
    {
        ...
        public void LoadProducts()
        {
            ...
            using (SqlConnection con = new SqlConnec-tion(connectionString))
            {
                try
                {
                    con.Open();
                    ...
                    using (SqlDataReader r = cmd.ExecuteReader())
                    {
                        while (r.Read())
                        {
                            ...
                            card.FillData(
                                ...
                                r["prN"]?.ToString(),
                                r["pvN"]?.ToString(),
                                r["price"] != DBNull.Value ? Con-vert.ToDecimal(r["price"]) : 0,
                                r["discount"] != DBNull.Value ? Con-vert.ToInt32(r["discount"]) : 0,
                                "шт.",
                                r["amountidstock"] != DBNull.Value ? Con-vert.ToInt32(r["amountidstock"]) : 0,
                                r["photo"]?.ToString()
                            );
