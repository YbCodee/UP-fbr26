...
namespace WinFormsApp1
{
    ...
    {
        string podkl = @"Data Source=.\SQLEXPRESS;Initial Cata-log=ybshoes;Integrated Security=True;TrustServerCertificate=True";
        int? _id;
        public Form5(int? id)
        {
            InitializeComponent();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Дата доставки не может быть раньше да-ты заказа");
                return;
            }
            using (SqlConnection conn = new SqlConnection(podkl))
            {
                ...
                string sql = (_id == null)
                ? @"INSERT INTO [Order]
                  (Crea-tionDate,DeliveryDate,PickUpPointId,StatusId,UserId,ReceiptCode)
        ...
        {
            if (_id == null) return;
            if (MessageBox.Show("Удалить заказ?", "Удаление", Message-BoxButtons.YesNo) == DialogResult.Yes)
            {
                using (SqlConnection conn = new SqlConnection(podkl))
                {
                    conn.Open();
