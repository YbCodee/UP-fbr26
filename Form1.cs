...
namespace WinFormsApp1 
{
    ...
    {
        // НИЖЕ СТРОКА ДЛЯ ПОДКЛЮЧЕНИЯ БД, КОТОРУЮ МЫ СОЗДАЛИ РАНЕЕ, НАЗВАНИЕ СЕРВЕРА ПОСМОТРИТЕ В СВОЙ-СТВАХ В SSMS 22(У МЕНЯ ТАКОЕ - WIN-PUURG92IVC5\SQLEXPRESS У ВАС МОЖЕТ НАЗЫВАТЬСЯ ИНАЧЕ!!!!)
        string connectionString = @"Data Source=.\SQLEXPRESS;Initial Catalog=ybshoes;Integrated Security=True;TrustServerCertificate=True";
        public Form1()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnec-tion(connectionString))
            {
                try
                {
                    conn.Open();
